...
class event_model extends CI_Model
{
   ...
   function getEventReports($event_id){
     $sql_str = "
     Select
     a.ticket_date_of_purchase,
     d.status,
     d.ticket_details_no,
     c.lastname,
     b.type,
     ...
     a.ticket_qrcode
 FROM
     ticket_purchased_history AS a
     LEFT JOIN ticket AS b ON (a.ticket_no = b.ticket_no)
     LEFT JOIN users AS c ON (
         a.ticket_hypercard_no = c.card_no
     )
     LEFT JOIN ticket_details d ON (a.ticket_qrcode = d.qr_code)
   ...
   function getOldEvent(){
     $sql_str = "
     select *from
      event
      where finished = 1 AND deleted = 0
     ";
     return $this->db->query($sql_str);
   }

   function checkActiveEvents() {
     $query = "SELECT COUNT(*) as noOfActiveEvents FROM event WHERE finished = 0 AND deleted = 0";
     $res = $this->db->query($query);
     if($res->row()->noOfActiveEvents == 0) {
       return false;
     } else {
       return true;
     }
